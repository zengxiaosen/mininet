     ...
     def __init__( self ):
	  ...
	  #num_hosts=16
          ...
          for i in range(0,num_access_switch):
               self.addLink(s_access[i],s_agg[i],bw=100)
          
          print 'sizes:',len(s_access) , len(s_agg)
          i=0
          while i<7:
               print i
               self.addLink(s_access[i],s_agg[i+1],bw=100)
               self.addLink(s_access[i+1],s_agg[i],bw=100)
               i=i+2
          ...
          for i in range(0,num_aggswitch):
               r=i%2
               if r==0:
                    self.addLink(s_agg[i],s_core[0],bw=100)
                    self.addLink(s_agg[i],s_core[1],bw=100)
               else:
                    self.addLink(s_agg[i],s_core[2],bw=100)
                    self.addLink(s_agg[i],s_core[3],bw=100)
          ...
          for i in range(0,num_access_switch):
               for j in range(0,8):
		#for j in range(0,2):
                    self.addLink(hosts[h],s_access[i],bw=100)
                    h=h+1
