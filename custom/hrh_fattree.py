...
class FatTreeTopo (Topo):
     def __init__( self ):
          ...
          for i in range(0,num_access_switch):
               self.addLink(s_access[i],s_agg[i],bw=20)
          
          print 'sizes:',len(s_access) , len(s_agg)
          i=0
          while i<7:
               print i
               self.addLink(s_access[i],s_agg[i+1],bw=20)
               self.addLink(s_access[i+1],s_agg[i],bw=20)
               i=i+2
          ...
          for i in range(0,num_aggswitch):
               r=i%2
               if r==0:
                    self.addLink(s_agg[i],s_core[0],bw=20)
                    self.addLink(s_agg[i],s_core[1],bw=20)
               else:
                    self.addLink(s_agg[i],s_core[2],bw=20)
                    self.addLink(s_agg[i],s_core[3],bw=20)
                    
          """ connect hosts to access switches """
          h=0
          for i in range(0,num_access_switch):
               for j in range(0,2):
                    self.addLink(hosts[h],s_access[i],bw=20)
                    h=h+1
